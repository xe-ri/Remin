...
def generate_academic_summary(user_query: str, source_ids: Optional[List[str]] = None) -> str:
    ...
    context_data = query_vector_db(retrieval_query, n_results=10, source_ids=source_ids)
    ...
    if not documents:
        return (
            ...
            "可能原因是检索到的片段主要来自图表、坐标轴或版面噪声；请重新勾选文献入库，或上传更清晰的 PDF 后再试。"
        )
...
"""
    ...
    try:
        ...
        return _fallback_academic_answer(user_query, documents, metadatas)
