...
export default function Home() {
  ...
  const [answerMeta, setAnswerMeta] = useState<ChatResult | null>(null);
  ...
  const handleAsk = async (event: FormEvent<HTMLFormElement>) => {
    ...
    if (answerMode !== "direct_llm" && selectedPaperIds.length === 0 && uploadedSources.length === 0) {
    ...
    try {
      const data = await requestJson<ChatResult>("/chat", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          query,
                        ...
                        <div className="flex items-start gap-4">
                          ...
                          <div className="flex-1">
                            ...
                            <div className="mt-4 flex flex-wrap gap-2 text-xs text-black/60">
                              ...
                              {(paper.domain_tags || []).map((tag) => (
                                <span key={`${paper.paperId}-${tag}`} className="rounded-full bg-amber-100 px-3 py-2 text-[var(--color-accent)]">
                                  {tag}
                                </span>
                              ))}
                            </div>
                            {paper.matched_domain_terms?.length ? (
                              <p className="mt-3 text-xs leading-6 text-black/50">
                                匹配术语：{paper.matched_domain_terms.join(", ")}
                              </p>
                            ) : null}
                          </div>
                        </div>
            ...
            <form onSubmit={handleAsk} className="glass-panel rounded-[2rem] p-6 sm:p-8">
              ...
              <div className="mt-6 flex flex-col gap-3 sm:flex-row">
                ...
                <select
                  ...
                  className="rounded-[1rem] border border-black/10 bg-white/80 px-4 py-3 text-sm outline-none"
                >
                  <option value="rag">RAG 证据回答</option>
                  <option value="direct_llm">直接 LLM 基线回答</option>
                </select>
              </div>
              ...
              <div className="mt-6 rounded-[1.2rem] border border-black/8 bg-white/70 p-4">
                ...
                {answerMeta ? (
                  <div className="mt-3 flex flex-wrap gap-2 text-xs text-black/55">
                    <span className="rounded-full bg-white/80 px-3 py-2">
                      模式 {answerMeta.answer_mode || "unknown"}
                    </span>
                    <span className="rounded-full bg-white/80 px-3 py-2">
                      证据数 {answerMeta.evidence_count ?? 0}
                    </span>
                    {(answerMeta.evidence_topics || []).map((topic) => (
                      <span key={topic} className="rounded-full bg-amber-100 px-3 py-2 text-[var(--color-accent)]">
                        {topic}
                      </span>
                    ))}
                  </div>
